...
def LengFunc(y,s,kappa0,nu,Tu,cz,qpllu0,alpha,radios,S,B,Xpoint,Lfunc):
    ...
    if radios["upstreamGrid"]:
        if s >S[Xpoint]:
            dqoverBds = ((nu**2*Tu**2)/T**2)*cz*Lfunc(T) - qpllu0/np.abs(S[-1]-S[Xpoint]) # second term is the converting of x point qpar to a heat source, which is constant
        else:
...
def returnImpurityFracLeng(constants,radios,S,indexRange,dispBassum = False,dispqassum = False,dispUassum = False, Ctol = 0.001, Ttol = 0.001):
    ...
    for i in indexRange:
        ...
        while np.abs(error0) > Ttol:
            ...
            while perChange > Ctol:

                
                #initial guess of qpllt, typically 0. This is a guess for 
                # q at virtual target (cold end of front). It's a rough 
                # approximation cause we're assuming a gamma_sheath
                ...
                if radios["upstreamGrid"]:
                    error1 = (qpllu1-0)/qpllu0 # If upstream grid, then want qpllu1 = 0 cause it's at midplane
                else:
                    error1 = (qpllu1-qpllu0)/qpllu0 # If no upstream grid, qpllu0 is at xpoint, aim to get qpllu1 the same
                if 0<error1:
                    switch1 = 0
